...
Core entities include Users (profiles, preferences, nutrition goals), Kitchen Inventory (items, expiration), Recipes (details, ingredients with tags array for dietary filtering), Meal Plans (scheduled meals, voting), Meal Plan Seats (configurable dining table, dietary restrictions, seat assignments), Recipe Interactions, Families (group management), Shopping Lists (collaborative tracking), Kitchen Equipment (ownership tracking, wishlists, brand/model details), Events (event-based meal planning), Nutrition Logs (daily nutrition tracking), Cooking Sessions (step-by-step cooking progress), and Chat Messages (AI history). These entities are interconnected through defined relationships to support the application's features.
...
**MVP+ Database Schema Extensions:**
- **Users Table Extensions:** Added `cookingLevel` enum (beginner, intermediate, advanced, expert), `householdSize` integer, nutrition goals (`dailyCalories`, `maxSodium`, `proteinTarget`, `carbsTarget`, `fatTarget`), and `onboardingCompleted` boolean for tracking user preferences.
- **Events Table:** Event-based meal planning with `eventName`, `scheduledFor` timestamp, `guestCount`, `invitedGuests` text array, `notes` text, and foreign keys to users and families. Indexed on `(userId, scheduledFor)` for performance.
- **EventMealPlans Junction Table:** Links events to multiple meal plans with unique constraint on `(eventId, mealPlanId)`.
- **NutritionLogs Table:** Daily nutrition tracking with `date` (date type for unique constraint), `totalCalories`, `totalProtein`, `totalCarbs`, `totalFat`, `totalSodium`, and auto-updated timestamps. Unique constraint on `(userId, date)` ensures one log per day.
- **NutritionLogMeals Table:** Granular meal-level nutrition tracking linked to nutrition logs with `mealName`, `recipeId`, nutrition breakdowns, and `loggedAt` timestamp. Indexed on `nutritionLogId` for efficient queries.
- **CookingSessions Table:** Tracks active cooking progress with `recipeId`, `currentStepIndex`, `status` enum (active, paused, completed, abandoned), `timers` JSONB array, and timestamps for session management. Indexed on `(userId, status)` for filtering active sessions.
...
*   **Home Dashboard (MVP+ Redesign):** Redesigned with calendar-centric layout featuring:
    - **CalendarStrip Component:** Horizontal scrolling 14-day calendar (7 past, 7 future) with date selection, auto-scroll to selected date, and visual indicators for meal plans (blue dots). Today's date highlighted with border. Smooth scroll animations with left/right navigation buttons.
    - **Today's Overview Widget:** Shows breakfast/lunch/dinner for selected date with recipe thumbnails, names, and meal type icons (Coffee, Sun, Moon). Clickable recipe cards link to detail pages. Empty state displays "No meal planned" for each meal type.
    - **My Plans Widget:** Displays next 4 upcoming meal plans with recipe thumbnails, time estimates ("in X hours"), family indicators, and nutrition badges (calories, prep time). Empty state shows "No upcoming meals planned" with link to meal planning page.
    - **My Kitchen Widget:** Inventory summary with total items count and expiring soon count (3-day window). Shows up to 2 expiring items with days remaining and category badges. Falls back to low stock items (≤2 quantity) if no expiring items. "View all items" link to kitchen inventory page.
    - **Suggested Recipes Widget:** Top 3 personalized recommendations with thumbnails, descriptions (2-line truncate), and nutrition badges. Empty state prompts user to add ingredients for suggestions.
    - **API Integration:** New endpoints `/api/meal-plans/by-date/:date` (meal plans for specific date) and `/api/nutrition-logs` (stub for future nutrition tracking). All widgets use TanStack Query for data fetching with proper loading and error states.
